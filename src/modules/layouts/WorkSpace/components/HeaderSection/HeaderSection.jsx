...
const HeaderSection = ({
  ...
  useEffect(() => {
    // useEffect que se ejecuta cada vez que cambia el texto del input
  ...
  const icon_clear = () => (
    ...
    />
  ); //Cada vez que se le de clic, se limpia el campo de busqueda.
  ...
  const phrases = [
    //Arreglo que almacena las frases a mostrar en 'Inicio', estas se muestran de forma aleatoria, con el indice de phrasesIndex.
  ...
  useEffect(() => {
    //Efecto que permite poner de forma aleatoria las frases.
  ...
  useEffect(() => {
    //useEffect que cambia el estado setIsMobile cuando el ancho es menor a 447.
  ...
  useEffect(() => {
    //Efecto que actualiza con cada cambio en buttonSelected, el nuevo botón cliqueado.
    ...
    <div className="header__Section">
      <div className="container__Search">
        <div
          ...
          style={{
            display: `${optionSelected != "Tutorias" ? "none" : "flex"}`,
          }}
        >
          ...
          <span
            ...
            style={{
              ...
              right: `${dataUser.userRol === "tutor" ? "145px" : "5px"}`,
            }}
          ...
          <button
            ...
            style={{
              display: `${
                optionSelected == "Tutorias" && dataUser.userRol == "tutor"
                  ? "flex"
                  : "none"
              }`,
            }}
            onClick={() => setVisCreateTutoring(true)}
          >
            {icon_pencil()} Crear Tutoria{" "}
          </button>
        </div>
        <button
          className="button--create"
        ...
        {/*Mostramos la frase según el indice guardado.*/}
      </div>
      <div className="container_buttons">
        {" "}
        {/*Contenedor que almacena el dropdown de Notificaciones y de Perfil. */}
        <Dropdown>
          ...
          <Dropdown.Menu className="container__notifications">
            {" "}
            {/*Contenedor que almacena el dropdown de notificaciones */}
            <section>
              <div
                className="title__notifications"
                  ...
                  <div>
                    {notifications.map((e, index) => (
                      ...
                      >
                        ...
                        {/*Mostramos el texto de la notificación */}
                      </Dropdown.Item>
                    ))}
                  </div>
                ) : (
        ...
        <Dropdown>
          {" "}
          {/*Bloque para el dropdown de Perfil. */}
          <Dropdown.Toggle variant="secondary" className="button--profile">
            {
              ...
              <Avatar
                ...
                sx={
                  !dataUser?.profileImage
                    ? stringAvatar(dataUser.username).sx
                    : undefined
                }
              >
                {!dataUser?.profileImage &&
          ...
          >
            ...
            >
              ...
              {/*Switche que permite el cambio de color en la aplicación */}
              {icon_moon()}
            </Dropdown.Item>
      ...
      {visCreateTutoring && (
        <CreateTutoring closeWindow={setVisCreateTutoring} />
      )}{" "}
      {/*Renderizamos el componente que nos permite crear las tutorias. */}
    </div>
  );
};
